...
- **A turn fails with `result_type=assistant last_content_type=none stop_reason=end_turn`** (≥ 0.2.2 shows the real cause instead) — the upstream model endpoint returned an empty response: the stream ended without any content block, usually a degraded or overloaded provider. Retry the turn; if it persists, switch models or start a fresh conversation. The bridge (≥ 0.2.2) reports this as a retryable `api_error` so the CLI retries automatically instead of failing silently.
